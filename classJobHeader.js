var db          = require('../db_schema/jobHeader');
...
function _DtGetData(req, loggedInUser, callback){
    ...
    if(strSearch){
		...
		if(!isNaN(strSearch)){
			Filters.push({'Production_Batch': Number(strSearch)});
			Filters.push({'Print_Batch': Number(strSearch)});
		}
	...
	db.countDocuments({}, function (err, c) {
		...
		db.countDocuments(searchStr, function(err, c) {
			...
			db.find(searchStr, 'Production_Batch Print_Batch Collection_Point Collection_Point_Ref created_at updated_at',
...
module.exports = class JobHeader {
    constructor(){}
